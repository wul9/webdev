...
function normalizePropertySchemaShorthand(info) {
    ...
    switch (presentation) {
        case "counter":
            ...
            (0, internal_1.assert)(type === "int", propError(info, "Counters cannot be used in collections."));
            break;
        default:
            break;
    }
...
function normalizePropertySchemaObject(info) {
    ...
    switch (presentation) {
        case "counter":
            (0, internal_1.assert)(type === "int", propError(info, "Counters can only be used when 'type' is 'int'."));
            break;
        default:
            break;
    }
    ...
    if (info.isPrimaryKey) {
        ...
        (0, internal_1.assert)(presentation !== "counter", propError(info, "Counters cannot be primary keys."));
        indexed = true;
    }
    const normalizedSchema = {
        name: info.propertyName,
...
function assertNotUsingShorthand(input, info) {
    ...
    (0, internal_1.assert)(shorthands.all.length === 0, propError(info, message));
}
/**
