...
const SyncConfiguration_1 = require("./SyncConfiguration");
...
/** @internal */
function isEstimateProgressNotificationCallback(callback) {
    return callback.length === 1;
}
exports.isEstimateProgressNotificationCallback = isEstimateProgressNotificationCallback;
function toBindingProgressNotificationCallback(callback) {
    if (isEstimateProgressNotificationCallback(callback)) {
        return (_, __, progressEstimate) => callback(progressEstimate);
    }
    else {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        return (transferredBytes, transferrableBytes, _) => callback(binding_1.binding.Int64.intToNum(transferredBytes), binding_1.binding.Int64.intToNum(transferrableBytes));
    }
}
...
function toBindingErrorHandler(onError) {
    return (sessionInternal, bindingError) => {
        ...
        const error = (0, errors_1.fromBindingSyncError)(bindingError);
...
function toBindingErrorHandlerWithOnManual(onError, onManual) {
    ...
    if (onError && onManual) {
        return toBindingErrorHandler((session, error) => {
            if (error instanceof errors_1.ClientResetError) {
...
function toBindingNotifyBeforeClientReset(onBefore) {
    return (internal) => {
        onBefore(new indirect_1.indirect.Realm(null, { internal }));
    };
}
...
function toBindingNotifyAfterClientReset(onAfter) {
    return (internal, tsr) => {
        onAfter(new indirect_1.indirect.Realm(null, { internal }), new indirect_1.indirect.Realm(null, { internal: binding_1.binding.Helpers.consumeThreadSafeReferenceToSharedRealm(tsr) }));
    };
}
...
function toBindingNotifyAfterClientResetWithFallback(onAfter, onFallback) {
    return (internal, tsr, didRecover) => {
        if (didRecover) {
            ...
            const realm = new indirect_1.indirect.Realm(null, {
...
function toBindingStopPolicy(policy) {
    if (policy === SyncConfiguration_1.SessionStopPolicy.AfterUpload) {
        return 2 /* binding.SyncSessionStopPolicy.AfterChangesUploaded */;
    }
    else if (policy === SyncConfiguration_1.SessionStopPolicy.Immediately) {
        return 0 /* binding.SyncSessionStopPolicy.Immediately */;
    }
    else if (policy === SyncConfiguration_1.SessionStopPolicy.Never) {
...
function toBindingClientResetMode(resetMode) {
    switch (resetMode) {
        case SyncConfiguration_1.ClientResetMode.Manual:
            return 0 /* binding.ClientResetMode.Manual */;
        case SyncConfiguration_1.ClientResetMode.DiscardUnsyncedChanges:
            return 1 /* binding.ClientResetMode.DiscardLocal */;
        case SyncConfiguration_1.ClientResetMode.RecoverUnsyncedChanges:
            return 2 /* binding.ClientResetMode.Recover */;
        case SyncConfiguration_1.ClientResetMode.RecoverOrDiscardUnsyncedChanges:
 ...
 */
const PROGRESS_LISTENERS = new Listeners_1.Listeners({
    add(callback, weakInternal, internal, direction, mode) {
        const token = internal.registerProgressNotifier(toBindingProgressNotificationCallback(callback), toBindingDirection(direction), mode === ProgressMode.ReportIndefinitely);
 ...
 * TODO: Consider adding a check to verify that the callback is removed from the correct SyncSession (although that would break the API)
 */
const CONNECTION_LISTENERS = new Listeners_1.Listeners({
...
class SyncSession {
    ...
    withInternal(cb) {
        return this.weakInternal.withDeref((syncSession) => {
            (0, assert_1.assert)(syncSession, "This SyncSession is no longer valid");
    ...
    get config() {
        return this.withInternal((internal) => {
            const appUser = binding_1.binding.Helpers.syncUserAsAppUser(internal.user);
            if (!appUser) {
                throw new Error("User is null");
            }
            const user = new User_1.User(appUser, mockApp);
     ...
     */
    get user() {
        const appUser = this.withInternal((internal) => binding_1.binding.Helpers.syncUserAsAppUser(internal.user));
        if (!appUser) {
            throw new Error("User is null");
        }
        return User_1.User.get(appUser);
    }
    /**
    ...
    addProgressNotification(direction, mode, callback) {
        assert_1.assert.function(callback, "callback");
    ...
    downloadAllServerChanges(timeoutMs) {
        return this.withInternal((internal) => new TimeoutPromise_1.TimeoutPromise(internal.waitForDownloadCompletion(), {
            ms: timeoutMs,
    ...
    uploadAllLocalChanges(timeoutMs) {
        return this.withInternal((internal) => new TimeoutPromise_1.TimeoutPromise(internal.waitForUploadCompletion(), {
            ms: timeoutMs,
    ...
    _simulateError(code, message, type, isFatal) {
        this.withInternal((internal) => binding_1.binding.Helpers.simulateSyncError(internal, code, message, type, isFatal));
    }
}
