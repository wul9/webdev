...
class Sync {
    ...
    static setLogLevel(app, level) {
        const numericLevel = (0, Logger_1.toBindingLoggerLevel)(level);
    ...
    static setLogger(app, logger) {
        const factory = binding_1.binding.Helpers.makeLoggerFactory((_, level, message) => {
            logger((0, Logger_1.fromBindingLoggerLevelToNumericLogLevel)(level), message);
        });
    ...
    static getAllSyncSessions(user) {
        return user.internal.app.syncManager.getAllSessionsFor(user.internal).map((session) => new SyncSession_1.SyncSession(session));
    }
    /**
    ...
    static getSyncSession(user, partitionValue) {
        (0, SyncConfiguration_1.validateSyncConfiguration)({ user, partitionValue });
        const config = (0, SyncConfiguration_1.toBindingSyncConfig)({ user, partitionValue });
        ...
        if (session) {
            return new SyncSession_1.SyncSession(session);
        }
        else {
            return null;
        }
    ...
    static initiateClientReset(app, path) {
        ...
        (0, assert_1.assert)(success, `Realm was not configured correctly. Client Reset could not be run for Realm at: ${path}`);
    }
    // TODO: Consider breaking the API, turning this into an instance method
    /**
    ...
    static openLocalRealmBehavior = {
        type: SyncConfiguration_1.OpenRealmBehaviorType.OpenImmediately,
    };
    /**
    ...
    static downloadBeforeOpenBehavior = {
        type: SyncConfiguration_1.OpenRealmBehaviorType.DownloadBeforeOpen,
        timeOut: 30 * 1000,
...
const SyncSessionNS = __importStar(require("./SyncSession"));
const BaseSubscriptionSetNS = __importStar(require("./BaseSubscriptionSet"));
const SubscriptionSetNS = __importStar(require("./SubscriptionSet"));
const SubscriptionNS = __importStar(require("./Subscription"));
const MutableSubscriptionSetNS = __importStar(require("./MutableSubscriptionSet"));
const LogLevelNS = __importStar(require("../Logger"));
// eslint-disable-next-line @typescript-eslint/no-namespace
(function (Sync) {
    Sync.ConnectionState = SyncSessionNS.ConnectionState;
    Sync.BaseSubscriptionSet = BaseSubscriptionSetNS.BaseSubscriptionSet;
    Sync.NumericLogLevel = LogLevelNS.NumericLogLevel;
    Sync.MutableSubscriptionSet = MutableSubscriptionSetNS.MutableSubscriptionSet;
    Sync.SubscriptionSet = SubscriptionSetNS.SubscriptionSet;
    Sync.SubscriptionSetState = BaseSubscriptionSetNS.SubscriptionSetState;
    /** @deprecated Please use {@link BaseSubscriptionSetNS.SubscriptionSetState | SubscriptionSetState} */
    Sync.SubscriptionsState = BaseSubscriptionSetNS.SubscriptionSetState;
    Sync.Subscription = SubscriptionNS.Subscription;
    Sync.Session = SyncSessionNS.SyncSession;
