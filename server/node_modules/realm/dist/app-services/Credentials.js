...
class Credentials {
    ...
    static anonymous(reuse = true) {
        return new Credentials(binding_1.binding.AppCredentials.anonymous(reuse));
    }
    static emailPassword(arg1, password) {
        if (typeof arg1 === "string") {
            assert_1.assert.string(password, "password");
            return new Credentials(binding_1.binding.AppCredentials.usernamePassword(arg1, password));
        }
        else {
            assert_1.assert.string(arg1.email, "email");
            assert_1.assert.string(arg1.password, "password");
            return new Credentials(binding_1.binding.AppCredentials.usernamePassword(arg1.email, arg1.password));
        }
    }
    /**
     ...
     */
    static apiKey(key) {
        return new Credentials(binding_1.binding.AppCredentials.apiKey(key));
    }
    /**
     ...
     */
    static apple(token) {
        return new Credentials(binding_1.binding.AppCredentials.apple(token));
    }
    /**
    ...
    static facebook(token) {
        return new Credentials(binding_1.binding.AppCredentials.facebook(token));
    }
    static google({ authCode, idToken }) {
        let internal;
        if (authCode !== undefined) {
            (0, assert_1.assert)(idToken === undefined, "Must not supply both an authCode or idToken field");
            internal = binding_1.binding.AppCredentials.googleAuth(binding_1.binding.GoogleAuthCode.make(authCode));
        }
        else {
            (0, assert_1.assert)(idToken !== undefined, "Must supply either an authCode or idToken field");
            internal = binding_1.binding.AppCredentials.googleId(binding_1.binding.GoogleIdToken.make(idToken));
        }
     ...
     */
    static jwt(token) {
        return new Credentials(binding_1.binding.AppCredentials.custom(token));
    }
    /**
    ...
    static function(payload) {
        return new Credentials(binding_1.binding.AppCredentials.function(payload));
    }
}
